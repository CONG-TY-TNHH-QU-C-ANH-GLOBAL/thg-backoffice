 ...
 * preflight, and this API answers no preflight (CORS is off). So the header's
 * presence proves the request came from a script this origin served.
 *
 ...
 * would buy nothing over this for a same-origin JSON API.
 *
 * ⚠ CONDITION: this reasoning holds because CORS is disabled and the API is
 * same-origin with its client. A deployment that opens CORS to another origin
 * must revisit both this guard and the SameSite value.
 */
